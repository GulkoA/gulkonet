...
description: Implicit Neural Representations (INRs) - representation of structured data as a continuous function. They intruduce novel methods of data compression, interpolation, and trend detection.
...
To evaluate the quality of an Implicit Neural Representation, we need to find how similar reconstructed data is to the original. Traditionally, we use metrics like PSNR (Peak signal-to-noise ratio) and SSIM (Structural similarity index measure). PSNR is essentially (omitting a few aspects of the formula) the inverse of normalized mean squared error, where $\lim_{PSNR \to 0}$ is no similarity at all and $ \lim_{PSNR \to \inf} $ is a perfect match. However, PSNR only takes global error into account, such that a reconstruction can look noisy, but still have high PSNR. This is why we pair it with SSIM, which considers relative luminance, contrast, and structure and approximates percieved quality difference. When training a neural network, we can introduce these metrics as parts of the loss function.
...
For instance, here I compressed 3D simulation of Isabel Hurricane[^5] with a computational compressor TTHRESH[^6], and an Implicit Neural Representation based on SIREN network[^7]:
...
| PSNR              | $\inf$    | 36.49     | 33.00     |
| SSIM              | 1         | 0.96      | 0.90      |

When training a traditional classifier or generative model, overfitting on training data is an absolute evil and should be avoided at all costs. However, since compressive INRs don't need to generalize, high overfitting is actually the goal!
...
[^3]: Wheeler RL, Gabbert F. Using Self-Generated Cues to Facilitate Recall: A Narrative Review. Front Psychol. 2017 Oct 27;8:1830. https://doi.org/10.3389/fpsyg.2017.01830. PMID: 29163254; PMCID: PMC5664228.
    {/* [^3]: Endel Tulving, Zena Pearlstone, Availability versus accessibility of information in memory for words, Journal of Verbal Learning and Verbal Behavior, Volume 5, Issue 4, 1966, Pages 381-391, ISSN 0022-5371, https://doi.org/10.1016/S0022-5371(66)80048-8. */}
